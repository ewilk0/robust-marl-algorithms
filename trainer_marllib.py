...
def make_env(scenario_name, shaped=True, noise_std=1.0, benchmark=False):
    ...
    env = marl.make_env(environment_name="mpe", map_name="simple_spread")[0]
    env.n = env.num_agents
    env.action_space = [env.action_space for _ in range(env.n)]
    env.observation_space = [env.observation_space["obs"] for _ in range(env.n)]
    return env
...
def evaluate(env, agents, num_runs, args, display=False, max_benchmark_iters=None):
    ...
    for _ in range(num_runs):
        obs = env.reset()
        obs = [obs[key]["obs"] for key in obs.keys()]
        done = False
        terminal = False
        episode_step = 0
        ...
        while not (done or terminal):
            ...
            actions = [np.argmax(c) for c in actions]
            actions = {f"agent_{i}": actions[i] for i in range(len(actions))}
            new_obs, rewards, dones, infos = env.step(actions)
            new_obs = [new_obs[key]["obs"] for key in new_obs.keys()]
            rewards = np.array(list(rewards.values()))
            dones = list(dones.values())
            cum_rewards += rewards
            cum_true_rewards += rewards
            done = all(dones)
...
def train(args):
    ...
    if args.evaluate:
        ...
        sr_mean, rewards, true_rewards = evaluate(
            env, agents, num_runs, args, display=args.render
        )
    ...
    while (episode_count <= args.max_episodes) and not terminated:
        ...
        while not (done or terminal):
            ...
            # act with all agents in environment and receive observation and rewards
            actions = [
                agent.act(o, explore=not args.evaluate) for o, agent in zip(obs, agents)
            ]
            actions = [np.argmax(c) for c in actions]
            actions = {f"agent_{i}": actions[i] for i in range(len(actions))}
            if args.debug:
                add_entropies(writer, obs, agents, train_step)
            new_obs, rewards, dones, _ = env.step(
                actions
            )  # for uncertain envs, the returned rewards are uncertain
            new_obs = [new_obs[key]["obs"] for key in new_obs.keys()]
            rewards = [c for c in rewards.values()]
            dones = [dones["__all__"] for _ in range(env.n)]
            done = all(dones)
            terminal = episode_step >= args.max_episode_len
            ...
            for i, agent in enumerate(agents):
                temp = list(actions.values())
                agent.experience(
                    episode_count, obs[i], temp[i], rewards[i], new_obs[i], dones[i]
                )
