...
import sys
sys.path.append("../robust_envs")
from robust_envs.utils import ParameterGenerator
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="maddpg",
        help="policy for good agents, choose from ddpg, maddpg, m3ddpg, or rmaddpg (robust-maddpg)",
    )
    parser.add_argument(
        "--adv-policy",
        type=str,
        default="maddpg",
        help="policy of adversaries, choose from ddpg, maddpg, m3ddpg, or rmaddpg (robust-maddpg)",
    )
    ...
    parser.add_argument(
        "--max-train-steps", type=int, default=500_000, help="maximum episode length"
    )
    parser.add_argument(
        "--eval-num-runs", type=int, default=365, help="number of runs on evaluation"
    )
    parser.add_argument(
        "--train-summer-pct", type=float, default=None, help="pct of summer dist for training env"
    )
    parser.add_argument(
        "--eval-summer-pct", type=float, default=None, help="pct of summer dist for eval env"
    )
    parser.add_argument(
        "--max-episodes", type=int, default=50_000, help="number of episodes"
    )
    parser.add_argument(
        "--max-episode-len", type=int, default=100, help="maximum episode length"
    )
    ...
    parser.add_argument(
        "--render", default=False, action="store_true", help="display agent policies"
    )
    parser.add_argument(
        "--eval",
        dest="evaluate",
        default=False,
        action="store_true",
        help="run agent policy without noise and training",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="simulation steps in between network updates",
    )
    parser.add_argument(
        "--noise-std",
        type=float,
        default=0.0,
        help="{0.0, 1.0, 2.0, 3.0, ...}, uncertainty level",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="discount factor for training of critic",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=1024,
        help="size of minibatch that is sampled from replay buffer",
    )
    parser.add_argument(
        "--hidden",
        type=int,
        default=64,
        help="number of hidden units in actor and critic",
    )
    parser.add_argument(
        "--memory-size", type=int, default=50_000, help="size of replay memory"
    )
    parser.add_argument(
        "--tau",
        type=float,
        default=0.01,
        help="update rate for exponential update of target network params",
    )

    # perturbing actions in m3ddpg
    parser.add_argument(
        "--adv-eps", type=float, default=1e-3, help="adversarial training rate"
    )
    parser.add_argument(
        "--adv-eps-s", type=float, default=1e-5, help="small adversarial training rate"
    )

    # Checkpointing
    parser.add_argument(
        "--exp-name", type=str, default="test", help="name of experiment"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="run number of experiment gets appended to log dir",
    )
    parser.add_argument(
        "--train-steps",
        type=int,
        default=1,
        help="how many train steps of the networks are done",
    )
    parser.add_argument("--save-every", type=int, default=365)
    ...
    parser.add_argument(
        "--sparse-reward", default=True, action="store_false", dest="shaped"
    )
...
def make_env(summer_percentage=None):
    params = {
    ...
    if summer_percentage is not None:
        params["stochastic_summer_percentage"] = summer_percentage
    params = ParameterGenerator(**params)
    env = MultiAgentBuildingEnv(params)
    env.n = env.num_agents
    env.action_space = list(env.action_spaces.values())
    env.observation_space = list(env.observation_spaces.values())
    return env
...
def create_agents(env, params):
    ...
    for action_space in env.action_space:
        ...
        else:  # for building env continuous spaces
            action_splits.append([np.prod(np.array(action_space.shape)) + 1])
            action_space_dim = np.prod(np.array(action_space.shape))
    ...
    for i in range(n_agents):
        ...
        # Critic
        n_critic_inputs = (
            n_obs_each[i] + n_actions_each[i]
            if policy_name == "ddpg"
            else sum(n_obs_each) + sum(n_actions_each)
        )
        ...
        # MADDPG agent
        agent = MADDPGAgent(
            i, "agent_%d" % i, env, actor, critic, params, policy_name, nature_actor
        )

        agents.append(agent)

    return agents, action_space_dim
...
def load_agent_states(dirname, agents, load_models=False, checkpoint=None):
    if checkpoint is None:
        ...
        checkpoint = max(
            [int(s.split("/")[-1].split(".pth")[0].split("_")[1]) for s in tars]
        )
    states_filename = os.path.join(dirname, "states_{}.pth.tar".format(checkpoint))
...
def evaluate(env, agents, num_runs, args, display=False, max_benchmark_iters=None):
    ...
    for i in range(num_runs):
        ...
        terminal = False
        episode_step = 0
        cum_rewards = np.zeros(len(agents), dtype=np.float64)
    ...
    return (
        dones_sum / num_runs,
        np.mean(rewards_all, axis=0),
        np.mean(true_rewards_all, axis=0),
    )
...
def add_entropies(writer, obs, agents, train_step):
    ...
    for o, agent in zip(obs, agents):
        ...
        agent_entropies = []
        for p in ps:
            agent_entropies.append(
                p.base_dist._categorical.entropy().detach().cpu().numpy()
            )
...
def log_writer(args, agents):
    ...
    if hasattr(agents[0].env.agents[0], "adversary"):
        run_name = "good_%s_adv_%s_noise_%.2f_" % (
            args.good_policy,
            args.adv_policy,
            args.noise_std,
        )
    ...
    run_name += (
        ...
        if args.exp_run_num == ""
        else "num_" + args.exp_run_num + "_" + str(np.datetime64("now"))
    )

    writer = SummaryWriter(
        log_dir=os.path.join("tensorboard-logs", args.exp_name, run_name)
    )
    ...
    for rewards_file in [train_rewards_file, eval_rewards_file]:
        if not os.path.isfile(rewards_file):
            with open(rewards_file, "w") as f:
                line = (
                    ",".join(["episode", "cum_reward"] + [a.name for a in agents])
                    + "\n"
                )
                f.write(line)
    ...
    files = {
        "train_rewards_file": train_rewards_file,
...
def train(args):
    ...
    train_env_summer_pct = args.train_summer_pct
    eval_env_summer_pct = args.eval_summer_pct
    env = make_env(summer_percentage=train_env_summer_pct)
    eval_env = make_env(summer_percentage=eval_env_summer_pct)
    agents, action_space_dim = create_agents(env, args)
    ...
    if args.evaluate:
        ...
        sr_mean, rewards, true_rewards = evaluate(
            eval_env, agents, num_runs, args, display=args.render
        )
        msg = "success rate: {:.3f}, cumsum return: {:.2f}, " + ", ".join(
            [a.name + ": {:.2f}" for a in agents]
        )
        print(msg.format(sr_mean, rewards.sum(), *rewards))
        print(
            "                             true rewards: "
            + ", ".join([a.name + ": {:.2f}" for a in agents]).format(*true_rewards)
        )
    ...
    if args.benchmark:
        ...
        fname = evaluate(
            ...
            args,
            display=False,
            max_benchmark_iters=args.benchmark_iters,
        )
    ...
    while (episode_count <= args.max_episodes) and not terminated:
        ...
        while not (done or terminal):
            ...
            # act with all agents in environment and receive observation and rewards
            # edited to take last n logits and shifted to populate box action space
            actions = [
                np.array(
                    [
                        agent.act(o, explore=not args.evaluate)[-action_space_dim:] * 2
                        - 1
                    ]
                )
                for o, agent in zip(obs, agents)
            ...
            new_obs, rewards, dones, _, _ = env.step(
                actions
            )  # for uncertain envs, the returned rewards are uncertain
            new_obs = list(new_obs.values())
            rewards = list(rewards.values())
            dones = list(dones.values())
            done = all(dones)
            ...
            for i, agent in enumerate(agents):
                agent.experience(
                    episode_count, obs[i], actions[i], rewards[i], new_obs[i], dones[i]
                )
            ...
            if train_step % args.train_every == 0:
                for _ in range(args.train_steps):
                    ...
                    # all_kls = []
                    for agent in agents:
                        actor_loss, critic_loss, model_loss, model_kls = agent.update(
                            agents
                        )
            ...
            '''if train_step % args.eval_every == 0:
                sr_mean, rewards, _ = evaluate(eval_env, agents, 10, args, display=False)
                if args.debug:
                    writer.add_scalar("success_rate", sr_mean, train_step)
                eval_msg(
                    start_time, train_step, episode_count, agents, sr_mean, rewards
                )'''
        ...
        if episode_count % args.save_every == 0:
            if episode_count % (5 * args.save_every) == 0:
                save_agent_states(
                    dirname, agents, episode_count, save_models=args.use_agent_models
                )

            # Keep track of final episode reward
            final_ep_reward = np.mean(cum_reward[-args.save_every :])
            train_rewards.append(final_ep_reward)
            final_ep_ag_rewards = [
                np.mean(rew[-args.save_every :]) for rew in agents_cum_reward
            ]
            ...
            with open(files["train_rewards_file"], "a") as f:
                line = (
                    ",".join(
                        map(str, [episode_count, final_ep_reward] + final_ep_ag_rewards)
                    )
                    + "\n"
                )
                f.write(line)
            ...
            sr_mean, rewards, _ = evaluate(eval_env, agents, args.eval_num_runs, args, display=False)
            eval_msg(
                start_time, train_step, episode_count, agents, sr_mean, rewards
            )
            ...
            with open(files["eval_rewards_file"], "a") as f:
                line = (
                    ",".join(map(str, [episode_count, rewards.sum()] + list(rewards)))
                    + "\n"
                )
                f.write(line)
            ...
            if args.debug:
                agent_rewards_dict = {
                    a.name: r for a, r in zip(agents, final_ep_ag_rewards)
                }
                writer.add_scalar("reward", final_ep_reward, episode_count)
                writer.add_scalars("agent_rewards", agent_rewards_dict, episode_count)

    # saving
    train_summer_pct = int(train_env_summer_pct * 100)
    eval_summer_pct = int(eval_env_summer_pct * 100) if eval_env_summer_pct is not None else None
    base_dir = "./ethan-results/"
    if not os.path.isdir(base_dir):
        os.mkdir(base_dir)
    save_dir = base_dir + f"train_summer_pct-{train_summer_pct}_eval_summer_pct-{eval_summer_pct}/"
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    robust_or_not = "robust" if args.good_policy == "rmaddpg" else "normal"
    save_filepath = save_dir + f"{int(time.time())}_saveevery-{args.save_every}_robust-{robust_or_not}"
    np.save(save_filepath+"_train", train_rewards)
    np.save(save_filepath+"_eval", eval_rewards)

    # plt.plot(train_rewards, label="train rewards")
    plt.plot(eval_rewards, label="eval rewards")
    plt.ylabel("eval reward")
    plt.title(f"Mean eval reward for {args.good_policy} across training episodes")
    plt.legend()
    plt.show()
...
def main():
    ...
    assert not (
        args.evaluate and args.benchmark
    ), "eval and benchmark cannot both be True"
    conf = (
        "conf:\n\tenv: %s\n\tgood vs adv: %s vs %s\n\tapprox actors: %r\n\tnoise_std: %.2f"
        % (
            args.scenario,
