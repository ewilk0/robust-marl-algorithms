...
class BuildingEnvModified(gym.Env):
    """BuildingEnv class.
    ...
    def reset(
        ...
        num_episodes = self.episode_len - 1
        if seed is None:
            episode = self.np_random.integers(low=0, high=self.length_of_weather-1)
        else:
            num_days_normalizer = ((self.episode_len * self.timestep) // 86_400) * 365
            episode = int((seed / num_days_normalizer) * self.length_of_weather) #  % (self.length_of_weather - 1)
            if episode >= self.length_of_weather:
                episode = self.length_of_weather - 1
        self.epoch = episode
        self.num_epoch_runs = 0
