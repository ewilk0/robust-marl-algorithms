...
class ReplayMemory:
    ...
    def sample_episodes_from(memories, batch_size, max_past=None):
        ...
        assert (
            len(set(b.last_episode for m in memories for b in m.buffers.values())) == 1
        )
        index = (
            memories[0].buffers["actions"].sample_episode_index(batch_size, max_past)
        )
    ...
    def sample_transitions_from(memories, batch_size, max_past=None):
        ...
        index = (
            memories[0].buffers["actions"].sample_transition_index(batch_size, max_past)
        )
    ...
    def add(self, episode_num, obs, action, reward, next_obs, done):
        """Saves a transition"""
        for buffer, data in zip(
            self.buffers.values(), [obs, action, reward, next_obs, done]
        ):
    ...
    def create_tensor(data, ensure_dim):
        ...
        tensor = torch.tensor(
            data, requires_grad=False, dtype=torch.float, device=DEVICE
        )
    ...
    def create_transition_batch(self, index):
        ...
        data = [
            self.create_tensor(b.sample_transitions(index), 2)
